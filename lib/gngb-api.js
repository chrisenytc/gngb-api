/*
 * gngb-api
 * https://github.com/chrienytc/gngb-api
 *
...
//Request Default
request = request.defaults({headers: {'User-Agent': 'chrisenytc'}});
...
exports.checkPackage = function (res, data, service, name, user) {
  request(getService(service, name, user), function (error, response, body) {
    if (!error && response.statusCode === 200) {
      data.exists = true;
      res.json(data);
    } else {
      res.json({error: 'Package '+name+' not found!', exists: false}, 404);
    }
  });
};
...
exports.createBadge = function (res, service, name, user) {
  request(getService(service, name, user), function (error, response, body) {
    if (!error && response.statusCode === 200) {
      ...
      res.json({error: 'Package '+name+' not found!'}, 404);
    }
  });
};
